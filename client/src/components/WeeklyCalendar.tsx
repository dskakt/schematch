...
import { format, addDays, startOfWeek, isSameDay, startOfDay, eachDayOfInterval, isBefore } from "date-fns";
...
export default function WeeklyCalendar({
  ...
  const diagonalBg = 'linear-gradient(45deg, transparent calc(50% - 0.5px), hsl(var(--border)) calc(50% - 0.5px), hsl(var(--border)) calc(50% + 0.5px), transparent calc(50% + 0.5px))';

  const renderDayHeader = (day: Date, padding: string) => {
    ...
    return (
      <div
        key={day.toISOString()}
        className={`${padding} text-center ${isToday ? 'bg-primary/10' : 'bg-muted'}`}
      ...
      >
        ...
        <div className="flex items-center justify-center">
          {isToday ? (
            <span className="bg-primary text-primary-foreground rounded-full flex items-center justify-center font-bold" style={{ width: padding === 'p-1' ? '1.5rem' : '1.75rem', height: padding === 'p-1' ? '1.5rem' : '1.75rem', fontSize: padding === 'p-1' ? '0.75rem' : '0.875rem' }}>
              {day.getDate()}
            </span>
          ) : (
            <span className={`font-medium ${padding === 'p-1' ? 'text-sm' : ''} ${isSunday ? 'text-red-600 dark:text-red-400' : isSaturday ? 'text-blue-600 dark:text-blue-400' : ''}`}>
              ...
              {weekDays.map((day) => {
                ...
                return (
                  <button
                    ...
                    className={`
                      p-1 transition-colors flex items-center justify-center font-black leading-none
                      ${isMobile ? 'text-xl' : 'text-2xl'}
                      ${isNoTimeSlot ? (isMobile ? 'min-h-[40px]' : 'min-h-[48px]') : (isMobile ? 'min-h-[28px]' : 'min-h-[32px]')}
                  ...
                  >
                    {selected && mode === "respond" && (
                      <div className={`rounded-full border-[3px] border-current flex-shrink-0 ${isMobile ? 'w-5 h-5' : 'w-6 h-6'}`} />
                    )}
                  </button>
                );
              })}
  ...
  return (
    <div className="space-y-4" data-testid="weekly-calendar">
      ...
      <div className="border rounded-lg bg-border relative">
        {/* Sticky date header — outside overflow-x-auto so page-level sticky works */}
        <div className="sticky top-0 z-20 bg-border">
          <div
            ref={headerScrollRef}
          ...
          >
            ...
            <div className="hidden sm:grid gap-px" style={{ gridTemplateColumns: `115px repeat(${weekDays.length}, minmax(45px, 1fr))` }}>
              ...
              {weekDays.map(day => renderDayHeader(day, 'p-2'))}
            </div>
          </div>
        </div>
