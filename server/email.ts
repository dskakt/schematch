...
interface SendPollVoteNotificationParams {
  organizerEmail: string;
  pollTitle: string;
  voterNames: string[];
  resultsLink: string;
  ...
  const emailContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="background-color: #e8f5e9; padding: 20px; border-radius: 8px; margin: 20px 0;">
        <h3 style="margin-top: 0; color: #2e7d32;">投票を作成しました</h3>
        <p style="margin: 10px 0; font-size: 16px;"><strong>${pollTitle}</strong></p>
      </div>
      ...
      <div style="background-color: #f5f5f5; padding: 20px; border-radius: 8px; margin: 20px 0;">
        ...
        <p style="margin: 10px 0;">このリンクを参加者に送信してください：</p>
        <p style="word-break: break-all; background-color: white; padding: 10px; border-radius: 4px;">
          <a href="${participantLink}" style="color: #2e7d32;">${participantLink}</a>
        </p>
      </div>
...
export async function sendPollVoteNotification({
  organizerEmail,
  pollTitle,
  voterNames,
  resultsLink,
}: SendPollVoteNotificationParams): Promise<void> {
  ...
  const emailContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="background-color: #e8f5e9; padding: 20px; border-radius: 8px; margin: 20px 0;">
        <h3 style="margin-top: 0; color: #2e7d32;">投票済み（${voterNames.length}名）</h3>
        <ul style="margin: 10px 0; padding-left: 20px;">
          ${voterList}
        </ul>
      </div>
...
${resultsLink}
  ...
  try {
    ...
    console.log(`✅ Poll vote notification email sent to ${organizerEmail}`);
  } catch (error) {
    console.error("Failed to send poll vote notification email:", error);
    throw error;
  }
}
